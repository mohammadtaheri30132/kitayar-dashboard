...
const ImportJsonPage = ({ onBack }: Props) => {
  ...
  const handleImport = async () => {
    ...
    setImporting(true); setFailedItems([]); setExpandedErrors(new Set()); setEditingJson(new Map()); setErrorTab('all')
    try {
      const res = await questionService.importBatch(parsedData, selectedBook)
      if (res.success) {
        setImportResult(res.data || res)
        if (res.data?.failedItems?.length > 0) { setFailedItems(res.data.failedItems); toast(`${res.data.success} موفق, ${res.data.failed} ناموفق`, { icon: '⚠️' }) }
        else toast.success(`✅ همه ${res.data?.success || 0} سوال import شدند`)
      }
  ...
  const handleForceImport = async (item: FailedItem) => {
    try {
      ...
      toast.success(`✅ سوال #${item.index} با موفقیت اضافه شد`)
      setFailedItems(prev => prev.filter(f => f.index !== item.index))
      setEditingJson(prev => { const n = new Map(prev); n.delete(item.index); return n })
    } catch (err: any) {
  ...
  const handleEditAndImport = async (item: FailedItem) => {
    try {
      ...
      toast.success(`✅ سوال #${item.index} با موفقیت اضافه شد`)
      setFailedItems(prev => prev.filter(f => f.index !== item.index))
    } catch (err: any) { toast.error('❌ JSON نامعتبر است. لطفاً اصلاح کنید.') }
  }
  ...
  const getStats = () => {
    ...
    const types: Record<string, number> = {}; const difficulties: Record<string, number> = {}; let composites = 0
    parsedData.forEach(q => { const hasSub = q.sub && Array.isArray(q.sub) && q.sub.length > 0; const t = hasSub ? 'ترکیبی' : (q.type || 'نامشخص'); types[t] = (types[t] || 0) + 1; difficulties[q.difficulty] = (difficulties[q.difficulty] || 0) + 1; if (hasSub) composites++ })
  ...
  const filteredErrors = failedItems.filter(item => {
    ...
    if (errorTab === 'duplicate') return ['DUPLICATE_ID', 'DUPLICATE_CONTENT', 'DUPLICATE_KEY'].includes(item.errorType)
    ...
    if (errorTab === 'other') return !['DUPLICATE_ID', 'DUPLICATE_CONTENT', 'DUPLICATE_KEY', 'VALIDATION', 'EMPTY_QUESTION', 'INVALID_TYPE', 'INVALID_DIFFICULTY', 'BOOK_NOT_FOUND'].includes(item.errorType)
    return true
  })

  const duplicateCount = failedItems.filter(f => ['DUPLICATE_ID', 'DUPLICATE_CONTENT', 'DUPLICATE_KEY'].includes(f.errorType)).length
  const validationCount = failedItems.filter(f => ['VALIDATION', 'EMPTY_QUESTION', 'INVALID_TYPE', 'INVALID_DIFFICULTY', 'BOOK_NOT_FOUND'].includes(f.errorType)).length
  ...
  const renderErrorItem = (item: FailedItem, idx: number) => {
    ...
    const isDuplicate = ['DUPLICATE_ID', 'DUPLICATE_CONTENT', 'DUPLICATE_KEY'].includes(item.errorType)
    const isValidation = ['VALIDATION', 'EMPTY_QUESTION', 'INVALID_TYPE', 'INVALID_DIFFICULTY', 'BOOK_NOT_FOUND'].includes(item.errorType)
    const bgColor = isDuplicate ? 'border-orange-300 bg-orange-50 hover:bg-orange-100' : isValidation ? 'border-red-300 bg-red-50 hover:bg-red-100' : 'border-red-200 bg-red-50 hover:bg-red-100'
          ...
          <div className="border-t border-gray-300 bg-gray-900 p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex gap-2">
                ...
                {item.canForce && (
                  <button onClick={(e) => { e.stopPropagation(); handleForceImport(item) }} className="text-xs text-orange-400 hover:text-orange-300 font-bold">⚡ افزودن اجباری</button>
                )}
        ...
        <div className="space-y-6">
          ...
          {importResult && (
            <div className={`rounded-xl p-5 ${importResult.failed > 0 ? 'bg-yellow-50 border border-yellow-200' : 'bg-green-50 border border-green-200'}`}>
              ...
              {importResult.failed > 0 && <div className="flex gap-3 text-xs">{duplicateCount > 0 && <span className="bg-orange-100 text-orange-700 px-2.5 py-1 rounded-full">🔄 {duplicateCount} تکراری</span>}{validationCount > 0 && <span className="bg-red-100 text-red-700 px-2.5 py-1 rounded-full">⚠️ {validationCount} اعتبارسنجی</span>}{otherCount > 0 && <span className="bg-gray-100 text-gray-700 px-2.5 py-1 rounded-full">❌ {otherCount} سایر</span>}</div>}
            </div>
          )}
            ...
            <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
              <div className="border-b border-gray-200 bg-gray-50 px-4 py-3">
                ...
                <div className="flex gap-1.5 mt-3">
                  ...
                  {duplicateCount > 0 && <button onClick={() => setErrorTab('duplicate')} className={`px-3 py-1.5 rounded-lg text-xs font-medium ${errorTab === 'duplicate' ? 'bg-orange-500 text-white' : 'bg-white text-orange-700 hover:bg-orange-50 border border-orange-200'}`}>🔄 تکراری ({duplicateCount})</button>}
