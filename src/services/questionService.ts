...
export interface QuestionData {
  ...
  type: string; difficulty: string; question: string; mainQuestion?: string
  ...
  createdAt?: string; is_composite?: boolean; sub?: any[]
  status?: string; tags?: string[]; has_image?: boolean
}
...
export const questionService = {
  getByBook: async (bookId: string, params?: any) => {
    const r = await api.get(`/questions/book/${bookId}`, { params })
    return r.data
  },
  ...
  create: async (data: Partial<QuestionData>) => {
    const r = await api.post('/questions', data)
    return r.data
  },
  update: async (id: string, data: Partial<QuestionData>) => {
    const r = await api.put(`/questions/${id}`, data)
    return r.data
  },
  ...
  importBatch: async (questions: any[], bookId?: string) => {
    const r = await api.post('/questions/import', { questions, bookId })
    return r.data
  },
  forceImport: async (question: any, bookId: string) => {
    const r = await api.post('/questions/force-import', { ...question, book: bookId })
    return r.data
  },
  batchUpdate: async (ids: string[], action: string, value?: string) => {
    const r = await api.post('/questions/batch', { ids, action, value })
    return r.data
  },
  updateStatus: async (id: string, status: string) => {
    const r = await api.patch(`/questions/${id}/status`, { status })
    return r.data
  },
  updateTags: async (id: string, tags: string[]) => {
    const r = await api.patch(`/questions/${id}/tags`, { tags })
    return r.data
  },
}
